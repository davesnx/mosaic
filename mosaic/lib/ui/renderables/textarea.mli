(** Interactive multi-line text input (code unit-based editing).

    Textarea provides a focusable multi-line text field with cursor navigation,
    placeholder text, and event callbacks for input, change, and submission.
    Keyboard input is handled automatically with printable ASCII keystrokes;
    programmatic updates can include any string. Mouse interactions are not
    handled.

    {1 Overview}

    Textarea uses a 2D scrolling viewport to display text that exceeds available
    width or height. The cursor is kept visible within the viewport. Text editing
    operates on UTF-8 code units. Enter inserts a newline; Ctrl+Enter submits.

    {1 Event Sequence}

    - [on_input]: Fired on every keystroke
    - [on_change]: Fired when focus is lost or Ctrl+Enter is pressed
    - [on_submit]: Fired when Ctrl+Enter is pressed *)
...
(** [apply_props textarea props] applies [props] to a mounted textarea using its
    setters, updating colors, placeholder, cursor appearance, length limits, wrap
    mode, and value. *)

val mount : ?props:Props.t -> Renderable.t -> t
(** [mount ?props node] configures [node] to render an interactive textarea. *)
...
val cursor_pos : t -> cursor_position
(** [cursor_pos t] returns the cursor position as [(row, col)] coordinates. *)

val hardware_cursor :
  t -> (int * int * Ansi.Color.t * cursor_style * bool) option
(** [hardware_cursor t] returns [(x, y, color, style, blinking)] for the
    terminal cursor in 1-based coordinates if the textarea is focused and visible;
...
(** [set_wrap_mode t mode] updates the text wrapping mode. *)
...
(** [on_change t handler] registers a callback fired when focus is lost or
    Ctrl+Enter is pressed, but only if text changed since last commit. *)

val on_submit : t -> (string -> unit) -> unit
(** [on_submit t handler] registers a callback fired when Ctrl+Enter is pressed.
*)

(** {1 Event routing} *)

val focus : t -> bool
(** [focus t] attempts to focus the textarea. Returns [true] if successful. *)

val blur : t -> unit
(** [blur t] removes focus from textarea. Fires [on_change] if text changed. *)
